...
from ..layers.lora_linear import Linear
...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert lora_config is not None, "lora_config is required"  
        ...
        if lora_config['apply_lora']:
            self.fc1 = Linear(in_features, hidden_features, r=lora_config['lora_r'], lora_alpha=lora_config['lora_alpha'], lora_dropout=lora_config['lora_dropout'], bias=bias[0])
            self.fc2 = Linear(hidden_features, out_features, r=lora_config['lora_r'], lora_alpha=lora_config['lora_alpha'], lora_dropout=lora_config['lora_dropout'], bias=bias[1])
        else:
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.pred_patches = pred_patches
        self.mask_flag = mask_flag
        if lora_config['apply_lora']:
            self.qkv = Linear(dim, dim * 3, r=lora_config['lora_r'], lora_alpha=lora_config['lora_alpha'], lora_dropout=lora_config['lora_dropout'], bias=qkv_bias)
            self.proj = Linear(dim, dim, r=lora_config['lora_r'], lora_alpha=lora_config['lora_alpha'], lora_dropout=lora_config['lora_dropout'], bias=qkv_bias)
        else:
    ...
    def get_prefix_casual_mask(self, N, prefix_len):
        mask = torch.eye(N, N, dtype=torch.bool, device='cuda')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.fused_attn:
            x = F.scaled_dot_product_attention(
                ...
                attn_mask=self.get_prefix_casual_mask(N, N-self.pred_patches) if self.mask_flag else None, # add prefix casual mask or not 
            )
        else:
            q = q * self.scale
